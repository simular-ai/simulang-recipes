    ...
    const groups = await Promise.all(
      files.map((f) =>
        f.type === 'application/pdf' || f.name.toLowerCase().endsWith('.pdf')
          ? pdfToImages(f).catch((err) => {
              setError(`PDF error: ${(err as Error).message}`)
              return []
            })
          : resizeImage(f)
              .then((a) => [a])
  ...
  return (
    <div className="chat-panel">
      <div className="chat-header">
        ...
        <button className="btn-ghost btn-small" onClick={handleReset} disabled={loading}>
          New chat
        </button>
      </div>
      ...
      <div className="chat-messages">
        {messages.length === 0 && (
          <p className="chat-empty">Upload a receipt photo or describe what you need — I'll update your list.</p>
        )}
          ...
          <div key={i} className={`chat-msg chat-msg-${msg.role}`}>
            {msg.attachmentPreviews?.map((src, j) =>
              src ? (
                ...
                <div key={j} className="chat-pdf-thumb chat-pdf-sent">
                  PDF
                </div>
              ),
            )}
            ...
            {msg.pendingRemovals && (
              <div className="chat-removal-confirm">
                {msg.removalOutcome ? (
                  ...
                  <>
                    <p>
                      <strong>Remove these items?</strong>
                    </p>
                    <ul>
                      {msg.pendingRemovals.map((item) => (
                        <li key={item.id}>{item.name}</li>
                      ))}
                    </ul>
                    <div className="chat-removal-actions">
                      <button className="btn-remove" onClick={() => handleRemovalDecision(i, true)}>
                        Remove
                      </button>
                      <button className="btn-ghost btn-small" onClick={() => handleRemovalDecision(i, false)}>
                        Keep
                      </button>
                    </div>
                  </>
          ...
          {attachments.map((att) => (
            <div key={att.key} className="chat-att-thumb">
              {att.preview ? (
                ...
                <div className="chat-pdf-thumb" title={att.name}>
                  PDF
                </div>
              )}
      ...
      <div className="chat-input-row">
        ...
        <button
          className="btn-ghost btn-small chat-attach-btn"
